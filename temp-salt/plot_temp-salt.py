...
if plot_type == 'final':
	...
	with sns.axes_style("darkgrid"):
		...
		dat['x'] = x
		plt.plot( 'x', 'mont_temp', data=dat, color='grey', marker='+', alpha=0.4, mew=2)
		ax1 = sns.regplot(x='x', y="mont_temp", data=dat, color='r', marker="+", scatter_kws={'alpha':0}, ci=95)
		plt.title('Monthly Mean Temperature')
		ax1.set(xticks=list(range(1994,2017,2)))
		ax1.set(xlabel='', ylabel='Temperature ($^\circ$C)')
		# __mont_region__
		fig2 = plt.figure(figsize=(20,3))
		x = list(range(0,len(dat.xtime)))
		x = [i/12 + 1994 for i in x]
		dat['x'] = x
		plt.plot( 'x', 'mont_salt', data=dat, color='grey', marker='+', alpha=0.4, mew=2)
		ax2 = sns.regplot(x='x', y="mont_salt", data=dat, color='g', marker="+", scatter_kws={'alpha':0}, ci=95)
		plt.title('Monthly Mean Salinity')
		ax2.set(xticks=list(range(1994,2017,2)))
		ax2.set(xlabel='', ylabel='Salinity (PSU)')
